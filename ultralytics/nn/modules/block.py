...
class EMA(nn.Module):
    ...
    def forward(self, x):
        ...
        #Split the conv1x1 into two equal parts
        x_attention, y_attention = torch.split(conv1x1, [W, H], dim=3)
...
class GhostC3k2(nn.Module):
    ...
    def __init__(self, c1, c2, n=2, c3k=False, e=0.5, g=1, shortcut=True):
        """Initializes the C3k2 module, a faster CSP Bottleneck with 2 convolutions and optional C3k blocks."""
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((1 + n) * self.c, c2, 1) 
        self.m = C3Ghost(self.c, self.c, n=1, shortcut=shortcut, g=g)
        self.n = n

    def forward(self, x):
        """Forward pass through C3k2 layer."""
        z = self.cv1(x).chunk(2, 1)
        y = [z[0], self.m(z[1])]
        y.extend(self.m(y[-1]) for _ in range(self.n-1))
        return self.cv2(torch.cat(y, 1))
